...
import type { queries, themeTypeForV } from "./config.js";
...
type CSSPayload = Partial<
  Record<keyof CSSStyleDeclaration | ExtraCSSDeclaration, CSSValue>
>;
type Queries = Record<string, CSSPayload>;

type CSSRules = CSSPayload & { query?: Queries } & Partial<{
...
function buildPayload(payload: CSSPayload) {
  return `${Object.entries(payload)
    .filter(([property]) => {
      return property !== "query" && !property.includes("&");
    })
    .map(([property, value]) => {
      return `${jsNameToCssName(property)}: ${
        value && typeof value === "object" && "variable" in value
          ? value.variable
          : value
      }`;
    })
...
export function css<TVariants extends Variants>(
      ...
      ? Array<{
          variants: Partial<{
            [TVariant in keyof TVariants]:
              | keyof TVariants[TVariant]
              | Array<keyof TVariants[TVariant]>;
          }>;
          styles: CSSRules;
        }>
      : never;
  ...
  variants?: TVariants extends Record<string, unknown>
    ? Partial<{
        [TKey in keyof TVariants]:
          | keyof TVariants[TKey]
          | (TVariants[TKey] extends { true: unknown } ? true : never)
          | (TVariants[TKey] extends { false: unknown } ? false : never);
      }>
    : undefined
) => string {
  let cssOut = `@layer base {${styles.base ? buildCSS(styles.base, ".&") : ""}}`;
  if (styles.variants) {
    cssOut += `@layer variants {${Object.keys(styles.variants)
      .map((variantName) => {
        return Object.entries(styles.variants![variantName]!)
          .map(([variantValue, value]) =>
            buildCSS(value, `.&.__${variantName}_${variantValue}`)
          )
          .join("\n");
      })
      .join("\n")}}`;
  ...
  if (styles.variants && styles.compoundVariants) {
    cssOut += `@layer compoundVariants{${styles.compoundVariants
      .map((compound) => {
        return `${Object.entries(compound.variants)
          .map(([key, value]) => {
            return `.&:is(${(Array.isArray(value) ? value : [value])
              .map((value) => `.__${key}_${value}`)
              .join(",")})`;
          })
  ...
  if (this?.addAsset) {
    ...
    throw new Error(
      "You need to make sure to import css function via `with {type: 'macro'}`"
    );
  }
...
export function styled<TTag extends ElementType, TVariants extends Variants>(
  this: MacroContext | void,
  tag: TTag,
  styles: {
    base?: CSSRules;
  ...
  React.PropsWithoutRef<
    ComponentProps<TTag> &
      (TVariants extends Record<string, unknown>
        ? Partial<{
            [TKey in keyof TVariants as `$${string & TKey}`]:
              | keyof TVariants[TKey]
...
> {
  ...
  if (this?.addAsset) {
    ...
    throw new Error(
      "You need to make sure to import styled function via `with {type: 'macro'}`"
    );
  }
...
type Options<
  TQueries extends Record<string, string>,
  TTheme extends CSSThing<string>,
> = {
  queries: TQueries;
  theme: Record<"base", TTheme> &
    Partial<Record<keyof TQueries, CSSThing<string>>>;
};

export function createConfig<
  const TQueries extends Record<string, string>,
  TTheme extends CSSThing<string>,
>(
  this: MacroContext | void,
  options: Options<TQueries, TTheme>
): {
  queries: TQueries;
  queriesTypeForM: TQueries;
  theme: {
    [TCategory in keyof TTheme]: Record<keyof TTheme[TCategory], Token>;
  };
  themeTypeForV: {
    [TCategory in keyof TTheme]: Record<keyof TTheme[TCategory], string>;
  };
} {
  // Here we make Typescript lie, Make sure the end of those loops match this type forever or bad stuff happens
  const defaultTheme = {} as {
    [TCategory in keyof TTheme]: Record<keyof TTheme[TCategory], Token>;
  };

  // Here we are just interested in the types exported to populate the v function options
  const themeTypeForV = {} as {
    [TCategory in keyof TTheme]: Record<keyof TTheme[TCategory], string>;
  };

  // Here we are just interested in the types exported to populate the m function options
  const queriesTypeForM = {} as {
    [TCategory in keyof TQueries]: TQueries[TCategory];
  };

  let cssOut = "@layer tokenMedia{";
  let cssBaseOut =
    "@layer tokenBase, tokenMedia, reset, base, variants, compoundVariants; @layer tokenBase {:root{";

  for (const themeMedia in options.theme) {
    if (themeMedia !== "base") {
      cssOut += `@${options.queries[themeMedia]}{:root{`;
    }
    for (const [category, tokensObject] of Object.entries(
      options.theme[themeMedia] as Record<string, CSSThing<string>>
    )) {
      if (themeMedia === "base")
      ...
      for (const [tokenName, tokenValue] of Object.entries(tokensObject)) {
        ...
        if (themeMedia === "base") {
          cssBaseOut += `${token}: ${tokenValue};`;
        } else {
          cssOut += `${token}: ${tokenValue};`;
        }
      }
    }
    if (themeMedia !== "base") {
  ...
  if (this?.addAsset) {
    ...
    throw new Error(
      "You need to make sure to import makeConfig function via `with {type: 'macro'}`"
    );
  }
...
export function q<K extends keyof typeof queries>(
  mediaQuery: K extends keyof typeof queries
    ? `${K}/${(typeof queries)[K]}`
    : never
) {
...
export function createTheme(
  ...
  if (this?.addAsset) {
    ...
    throw new Error(
      "You need to make sure to import createTheme function via `with {type: 'macro'}`"
    );
  }
